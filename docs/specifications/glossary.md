...
Orca Agent Memory preserves conversation continuity across Codex sessions in
Phase 1, across agents in Phase 2, and across devices in Phase 3. Its distilled
context lives alongside durable knowledge in the Orca LLM wiki vault.
...
**Orca LLM Wiki**:
The Owner's vault and knowledge workflows for durable information and a second
brain. It is the source of truth for accepted knowledge and also houses the
designated conversation-memory area.

**Orca Agent Memory**:
The system that distills conversation history, stores useful context in the
designated vault area, and makes it available for continuing discussions and tasks.
...
**Recall Request**:
An Owner- or agent-initiated memory question, its scope or kind filters, and only
...
**Active Recall**:
Retrieval initiated because the Owner asks to remember or resume a discussion.

**Passive Recall**:
Retrieval initiated by the agent when earlier context is relevant to the current
discussion. Phase 1 uses the same bounded, scoped recall interface; startup
provides invocation guidance rather than automatically selecting memory content.
...
- **Exact behavior:** [Memory System Contract](memory-system-contract.md)
