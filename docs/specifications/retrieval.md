...
1. Semantic memory recall MUST occur only through a scoped recall invocation.
   Active recall answers an Owner request. Passive recall is Codex initiating
   that same interface when earlier context is relevant to the current work;
   it is not a separate background search or permission to scan unrelated scopes.
   Startup provides recall instructions without preloading conversation content.
2. Recall MUST apply authority, visibility, project, and status filters before
   ranking. Ordinary requests require a resolved project or explicit `general`
   or `unassigned` scope. An exact conversation key may supply scope only when it
   identifies one indexed continuation. Contradictory selectors MUST be rejected.
   The CLI resolves an omitted project from the exact mapped workspace root;
   an unmapped workspace MUST NOT become an all-project search.
...
An exact continuation MUST be selected before associated material and return
its complete meaning when it fits the existing token budgets. If a lower budget
requires truncation, omission MUST be visible. Exact identity must not lose its
result slot to a higher-scoring record that merely mentions it.
...
[Current](../project-record/current.md) owns the available Recall surfaces and
pending scope findings. Canonical-first ordering applies only among comparably
relevant selected results, as specified above.
...
- **Requirements:** [Recall and retrieval](requirements.md#recall-and-retrieval)
- **Memory semantics:** [Memory Model](memory.md)
- **Candidate exclusion:** [Knowledge Candidates](knowledge-candidates.md)
- **Integration Seam:** [Integration Architecture](../architecture/integrations.md)
- **Historical budget rationale:** [Recall Context Budget](../archive/research/recall-context-budget.md)
