...
title: Orca Memory Model Specification
...
`docs/archive/legacy-docs/memory-record-contract.md`.

## This document owns

- Typed Memory Record and structural-summary schemas and bodies.
- Memory identity, project identity, conflict/review state, filename/layout policy, and controlled relationships.

## This document does not own

- Capture, processing schedule, recall ranking, ordinary Knowledge Candidate lifecycle, or implementation progress.

> [!NOTE]
> This specification is the normative owner of Typed Memory Record and Storage behavior.
...
decision history, preference profile, or canonical project record. It is updated
cumulatively from its prior revision plus newly processed turns: preserve useful
earlier outcomes and their reasons, tentative ideas labelled as ideas, unresolved
questions, and next steps. Resolved or changed positions must remain clearly
labelled when their history matters for continuation. A useful summary does not
require a Typed Memory Record or a Knowledge Candidate. This remains a bounded
distillation, not a promise to retain every historical detail or prior wording.
A branch has its own `conversation_id` and therefore its own file. The configurable default maximum
is 2,000 tokens.
...
The provider proposes `kind`, `subject`, `status`, and Workstream Labels.
Processor binds each operation to its cited Owner segments and derives
`source_updated_at` from those trusted source timestamps; provider-supplied
dates do not establish authority. Governance supplies the resolved
...
For a Project Summary published with changed records, Storage inserts the
newly allocated record IDs into its relevant-memory list before validation.
The provider must not guess those future identities. Supplied existing IDs
remain subject to same-project and existing-record validation.
