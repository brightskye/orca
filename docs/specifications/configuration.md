...
Backup verification and explicit standalone backup staging do not load these
sources. Their configuration-independent recovery boundary is owned by the
[Backup specification](backup.md). Backup creation and normal staging still
...
The vault and Codex rollout store MUST be outside every configured project root
and detected Git worktree. Compare normalized paths, including symlink targets.
Partial or unreadable Git markers fail closed; an empty `.git` directory alone
does not prove a checkout. Runtime state may remain in the existing ignored
checkout-local `.runtime/` layout and must still stay outside the vault.
...
|---|---|---|
| `processor` | `model_window_tokens`, `input_tokens`, `output_tokens`, `new_evidence_tokens`, `preceding_overlap_tokens`, `continuation_summary_tokens`, `project_summary_tokens`, `related_records_tokens`, `max_related_records` | [Processing Pipeline](processing.md#context-budget) |
| `recall` | `total_tokens`, `per_document_tokens`, `exact_continuation_tokens`, `max_results` | [Retrieval Contract](retrieval.md#result-budget-and-excerpts) |
...
Implemented configuration support and known path-validation gaps belong in
...
- **Requirements:** [REQ-OPS-001](requirements.md#req-ops-001--local-configuration-boundary)
- **Deployment:** [Deployment Architecture](../architecture/deployment.md)
- **Data ownership:** [Data Architecture](../architecture/data.md)
- **Security:** [Security and Trust](../architecture/README.md#security-and-trust)
- **Operator procedures:** [Phase 1 Local Runbook](../operations/runbook.md)
